...
import os
import scipy
from model.simnet_13 import SIMNET

                          
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
from util.dataset_pair_cls_mask import BaseDataset
...
def write_log(log_dir):
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    filename = log_dir + '/simnet_13_all.log'
    logging.basicConfig(
        filename=filename,
        level=logging.INFO,
    )
...
def net_train(fold, net, train_loader):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (v_content_1, v_content_2, label)  in enumerate(train_loader):
            vid_1 = v_content_1[0]
            frame_feature_1 = v_content_1[1][0][0].to(device)
            frame_mask_1 = v_content_1[1][0][1].to(device)
            text_1 = v_content_1[1][1]

            input_id = text_1[0].to(device)
            token_type_ids = text_1[1].to(device)
            attention_mask = text_1[2].to(device)
            embedding_1 = net((input_id,token_type_ids,attention_mask), (frame_feature_1, frame_mask_1))
            embedding_1 = F.normalize(embedding_1, p=2, dim=1)

            vid_2 = v_content_2[0]
            frame_feature_2 = v_content_2[1][0][0].to(device)
            frame_mask_2 = v_content_2[1][0][1].to(device)
            ...
            embedding_2 = net((input_id, token_type_ids, attention_mask), (frame_feature_2, frame_mask_2))
            ...
            net.train()
            total_batch += 1
        torch.save(net.state_dict(), 'checkpoint/data_new_simnet_17_best_3e-04_without_drop_unsup_all_epoch_{}.pth'.format(epoch))
    return net
...
if __name__ == "__main__":
    ...
    train_dataset = BaseDataset(df_train, data_path_list, text_type='com_text')
    train_loader = DataLoader(train_dataset, batch_size=batch_size,num_workers=8,pin_memory=True, shuffle=True)
    net = SIMNET(device, ckpt_text='checkpoint/pretrain_model/text_model_com_focal_2_without_pair.pth', ckpt_video='checkpoint/pretrain_model/video_model_trans.pth').to(device)
