...
def write_log(log_dir):
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    filename = log_dir + '/multi_bert_tag.log'
    logging.basicConfig(
        filename=filename,
        level=logging.INFO,
    )
...
def net_train(fold, net, train_loader, dev_iter):
    ...
    for epoch in range(max_epoch):
        ...
        net.train()
        for i, (vid, frame, text, tag) in enumerate(train_loader):
            tag = tag.to(device)
            ...
            loss, prob, _ = net((input_id,token_type_ids, attention_mask), (video_feature, video_mask), tag)
        ...
        if dev_best_acc < dev_acc:
            ...
            torch.save(net.state_dict(), 'checkpoint/pretrain_model/multi_bert_tag.pth') 
...
def evaluate_spearman(net, data_loader):
    ...
    embedding_list = []
    with torch.no_grad():
        for vid, frame,text, tag in data_loader:
            ids = vid
            tag = tag.to(device)
            video_feature = frame[0].to(device)
            video_mask = frame[1].to(device)
            ...
            loss, prob, embedding= net((input_id,token_type_ids, attention_mask), (video_feature, video_mask), tag)
            loss_val = loss.item()
            loss_total += loss_val
...
if __name__ == "__main__":
    ...
    train_dataset = BaseDataset(train_data_path_list, desc_file, text_type='title', training=True)
    train_loader = DataLoader(train_dataset, batch_size=batch_size,num_workers=8,pin_memory=True, shuffle=True)

    eval_dataset = BaseDataset(eval_data_path_list, desc_file, text_type='title', training=True)
