...
from util.dataset_point_cls_mask import BaseDataset
...
def write_log(log_dir):
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    filename = log_dir + '/video_model_6.log'
    logging.basicConfig(
        filename=filename,
        level=logging.INFO,
    )
...
def net_train(fold, net, train_loader, dev_iter):
    ...
    for epoch in range(max_epoch):
        ...
        net.train()
        for i, (vid, frame, _, tag) in enumerate(train_loader):
            tag = tag.to(device)
            video_feature = frame[0].to(device)
            video_mask = frame[1].to(device)
            loss, prob, _ = net((video_feature, video_mask), tag)
        ...
        if dev_best_acc < dev_acc:
            ...
            torch.save(net.state_dict(), 'checkpoint/pretrain_model/video_model_trans.pth') 
...
def evaluate_spearman(net, data_loader):
    ...
    embedding_list = []
    with torch.no_grad():
        for vid, frame, _,tag in data_loader:
            ids = vid
            tag = tag.to(device)
            video_feature = frame[0].to(device)
            video_mask = frame[1].to(device)
            loss, prob, embedding= net((video_feature, video_mask), tag)
            loss_val = loss.item()
            loss_total += loss_val
